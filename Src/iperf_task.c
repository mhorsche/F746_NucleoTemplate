 ...
 * @brief A FreeRTOS+TCP demo program:
...
/* Stack size needed for prvIPerfTask(), a bit of a guess: */
#define ipconfigIPERF_STACK_SIZE_IPERF_TASK 680
#endif

#ifndef ipconfigIPERF_PRIORITY_IPERF_TASK
/* The priority of prvIPerfTask(). Should be lower than the IP-task
...
static void prvIPerfTask(void *pvParameter);

static void prvIPerfServerWork(Socket_t xSocket);
...
static Socket_t prvCreateTCPServerSocket();
#endif

#if (ipconfigIPERF_HAS_UDP != 0)
static Socket_t prvCreateUDPServerSocket();
#endif
...
void vIPerfInstall(void)
{
  ...
  if (pxIperfTask == NULL)
  {
    ...
    xTaskCreate(prvIPerfTask, "IPerf", ipconfigIPERF_STACK_SIZE_IPERF_TASK, NULL, ipconfigIPERF_PRIORITY_IPERF_TASK, &pxIperfTask);
  }
}
/*-----------------------------------------------------------*/
...
static void prvIPerfServerWork(Socket_t xSocket)
{
  ...
  if ((xNexSocket != NULL) && (xNexSocket != FREERTOS_INVALID_SOCKET))
  {
    ...
    FreeRTOS_printf(("prvIPerfTask: Received a connection from %s:%u\n",
                     pucBuffer,
...
static void prvIPerfTCPWork(TcpClient_t *pxClient)
{
  ...
  {
    xRecvResult = prvIPerfTCPSend(pxClient);
  }
...
#if (ipconfigUSE_CALLBACKS == 0) && (ipconfigIPERF_HAS_UDP != 0)
static void prvIPerfUDPWork(Socket_t xSocket)
{
  BaseType_t xRecvResult;
  struct freertos_sockaddr xAddress;
...
static Socket_t prvCreateTCPServerSocket()
{
  ...
  FreeRTOS_printf(("prvIPerfTask: created TCP server socket %p bind port %u: %ld listen %ld\n",
...
static Socket_t prvCreateUDPServerSocket()
{
  ...
  FreeRTOS_printf(("prvIPerfTask: created UDP server socket %p bind port %u: %ld\n",
...
#else
  {
    ...
    xHandler.pOnUdpReceive = prvOnUdpReceive;
...
#if (ipconfigIPERF_HAS_TCP != 0)
  {
    xTCPServerSocket = prvCreateTCPServerSocket();
  }
#endif /* ipconfigIPERF_HAS_TCP */
...
#define HUNDREDTH_MB ((1024 * 1024) / 100)
  for (;;)
  {
    ...
    if (xResult != 0)
    {
      ...
      prvIPerfServerWork(xTCPServerSocket);
    ...
    if (xResult != 0)
    {
      prvIPerfUDPWork(xUDPServerSocket);
    }
    else
#endif /* ipconfigUSE_CALLBACKS */
    {
